...
% $$$   [~,ne] = ode45(@(t,ne) dntdt_fcn(t,ne,alpha,q),[0 dt],ne0);
% $$$   ne = ne(end,:).';
 case 'integrate'
...
% $$$   end
% $$$   ndt = ndt/ndt(end);
% $$$   [~,ne] = ode45(@(t,ne) dntdt_fcn(t,ne,alpha,q),ndt*dt,ne0);
% $$$   ne = trapz(ndt*dt,ne).'/dt;
 case 'equilibrium'
  ne = sqrt(q./alpha);
 case 'linearend'
...
function dnedt = dntdt_fcn(t,ne,alpha,q)
% DNTDT_FCN - 
%   
dnedt = q - alpha.*ne.^2;
