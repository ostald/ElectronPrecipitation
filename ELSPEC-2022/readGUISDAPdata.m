...
if ~isempty(fitdir)
    ...
    for k=1:ntpar
        ii = ~isnan(hpar(:,k));
        h_curr = [0;hpar(ii,k);1e6];
        par_curr = par1(ii,:,k);
        par_curr = [par_curr(1, :);par_curr;par_curr(end, :)];
        parstd_curr = parstd1(ii,:,k);
        parstd_curr = [parstd_curr(1, :);parstd_curr;parstd_curr(end, :)];
        % interpolate, some tricks at edges
        par2(:,:,k) = interp1(h_curr,par_curr,h,'linear','extrap');
        parstd2(:,:,k) = interp1(h_curr,parstd_curr,h,'linear','extrap');
    end
    for k=1:nh
        for l=1:4
            par3(k,l,:) = interp1([0;tepar;1e20],[squeeze(par2(k,l,1));squeeze(par2(k,l,:));squeeze(par2(k,l,end))],te,'linear','extrap');
            parstd3(k,l,:) = interp1([0;tepar;1e20],[squeeze(parstd2(k,l,1));squeeze(parstd2(k,l,:));squeeze(parstd2(k,l,end))],te,'linear','extrap');
        end
    end
    for l = 1:4
        par(:, l , :) = inpaint_nans(squeeze(par3(:, l, :)), 4);
        parstd(:, l , :) = inpaint_nans(squeeze(parstd3(:, l, :)), 4);
    end
 end
...
% 
%     figure
%     imagesc(tmidpar, hpar(:, 1), squeeze(par1(:, 3, :)))
%     colorbar
%     set(gca,'YDir','normal')
% 
%     nhpar = size(hpar, 1);
...
% 
%     figure
%     imagesc(tpar_, hpar(:, 1), squeeze(par3(:, 3, :)))
%     colorbar
%     set(gca,'YDir','normal')
% 
...
% 
%     figure
%     imagesc(tpar_, hpar_, squeeze(par2(:, 3, :)))
%     colorbar
%     set(gca,'YDir','normal')
% 
...
% 
%     figure
%     imagesc(tmid, h, squeeze(par(:, 3, :)))
%     colorbar
%     set(gca,'YDir','normal')
% 
% 
% 
%     if any(squeeze(par(:, 3, :))<0, 'all')
%         keyboard
%     end
% 
% 
