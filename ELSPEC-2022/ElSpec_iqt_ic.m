...
% 5th collect output
n_t = numel(out.ts);
out.polycoefs = polycoefs_all(:,1:n_t);
out.best_order = best_order_all(1:n_t);
out.params = n_params_all(1:n_t);
out.ne = ne_all(:,1:n_t);
out.neEnd = neEnd_all(:,1:n_t);
out.Ie = Ie_all(:,1:n_t);
...
xCovar = ElSpec_error_estimate_polycoefs( out.pp(:,1:out.nSteps(1)), ...
                                          ppstd(:,1:out.nSteps(1)), ...
                                          ne0, ...                          => should be out.ne0(:, 1)? OS
                                          ne0Cov, ...
                                          A, ...
                                          out.alpha(:,1), ...
                                          out.dt(1:out.nSteps(1)), ...
                                          out.Ec, ...
                                          out.dE, ...
                                          'endNe', ...% TO-BE-FIXED!        => what is to be fixed? OS
                                          out.ieprior, ...
                                          out.stdprior, ...
                                          polycoefs_all{best_order_all(1),1},... 
                                          numel(out.pp(:,1:out.nSteps(1))), ...
                                          out.ErrType, ...
                                          out.ErrWidth ...
                                          );
...
while it+out.nSteps(it)<=numel(out.te)
  ...
  ne0 = out.ne(:, it+[0:out.nSteps(it)-1])
  
  xCovar = ElSpec_error_estimate_polycoefs( out.pp(:,it+[0:out.nSteps(it)-1]), ...
                                            out.ppstd(:,it+[0:out.nSteps(it)-1]),...
                                            ne0, ...   % These buggers              => should be out.ne0? OS => also update to the correct timestep
                                            ne0Cov,... % are tricky!                => same here, but needs to be calculated first
                                            A, ...
                                            out.alpha(:,it+[0:out.nSteps(it)-1]), ...
                                            out.dt(it+[0:out.nSteps(it)-1]), ...
                                            out.Ec, ...
                                            out.dE, ...
                                            out.integtype, ...
                                            out.ieprior, ...
                                            out.stdprior, ...
                                            polycoefs_all{best_order_all(it),it},...
                                            numel(out.pp(:,it+[0:out.nSteps(it)-1])), ...
                                            out.ErrType, ...
                                            out.ErrWidth ...
                                            );
  
  % flux covariance matrix
  currIeCov = ElSpec_error_estimate_Ie( xCovar(1:(out.best_order(it)+1),...
                                        1:(out.best_order(it)+1)), ...
                                        polycoefs_all{best_order_all(it),it}, ...
                                        out.Ec );
  currIeStd = sqrt(diag(currIeCov));
  for tt = it:(it+out.nSteps(it)-1)
    out.IeCov(:,:,tt) = ElSpec_error_estimate_Ie( xCovar(1:(out.best_order(it)+1),...
                                                         1:(out.best_order(it)+1)), ...
                                                  polycoefs_all{best_order_all(it),it}, ...
                                                  out.Ec );
    out.IeStd(:,tt) = sqrt(diag(out.IeCov(:,:,tt)));
    % out.IeCov(:,:,tt) = currIeCov;
    % out.IeStd(:,tt) = currIeStd;
  end

  [currneEndStd, currneEndCov] = ElSpec_error_estimate_ne2( ne0, ...
                                                            polycoefs_all{best_order_all(it),it}, ...
                                                            xCovar, ...
  ...
  ne0Cov = currneEndCov
  %take currneEndCov for input for next  ne0Cov
  %collect output


  % for tt = it:(it+out.nSteps(it)-1)
  %   % [out.neEndStd(:,tt) , out.neEndCov(:,:,tt)] = ...
  ...
  %   %                              out.alpha(:,(it+out.nSteps(it)-1)), ...
  %   %                              'endNe' );
  %   out.neEndStd(:,tt) = currneEndStd;
  %   out.neEndCov(:,:,tt) = currneEndCov;
  % 
  %end
...
% Characteristic energy/centre-of-energy
for it = numel(out.ts):-1:1
  out.E_char(it) = sum(out.Ie(:,it).*out.dE'.*out.Ec')./sum(out.Ie(:,it).*out.dE');
end
...
% xOut.best_orderX, xOut.IeX, xOut.neX, xOut.neEndX
  ...
  lsq_opts = optimset('lsqnonlin');
  lsq_opts.Display = 'off';%'off';%'final';
  lsq_opts.MaxFunEvals = 1e4;
  lsq_opts.MaxIter = 1e6;
  lsq_opts.TolFun = 1e-8;
  lsq_opts.TolX = 1e-8;
...
% $$$     end
    ...
    if ~isempty(ieprior)
      ...
      % [x,fval,~,exitflag] = lsqnonlin( @(p) ElSpec_resfun(p, ...
      %                                                     pp, ...
      %                                                     ppstd, ...
      %                                                     ne00, ...
      %                                                     A, ...
    ...
    %                                        x, X0min, X0max, lsq_opts);
    %   catch
    %   lsq_opts.Algorithm = 'levenberg-marquardt';
    %   [x,fval,~,exitflag] = lsqnonlin( @(p) ElSpec_resfun(p, ...
    %                                                       pp, ...
    %                                                       ppstd, ...
    %                                                       ne00, ...
    ...
    %                                                       S_type), ...
    %                                        x, X0min, X0max, lsq_opts);
    %   lsq_opts.Algorithm = 'trust-region-reflective';
    %   end
    % 
    %   [x,fval,exitflag] = fminsearchbnd( @(p) ElSpec_fitfun(p, ...
    %                                                     pp, ...
