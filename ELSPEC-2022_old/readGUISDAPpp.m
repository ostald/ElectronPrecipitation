...
load(ff{1});
i_end = find(diff(r_pprange)<0);
i_end = [i_end(1:1:end); length(r_pprange)]; %i_end = i_end(1:3:end); changed 3 -> to use all power profiles. 
% Depending on data set, there are 3 minor peaks following each major. 
% I only have major peaks, so use all data.
...
for k = 1:length(ff)
    ...
    % start indices of profiles
    i2 = [1;i_end(1:1:end-1)+1];

    % end indices of profiles
    i3 = [i_end(1:1:end)];%length(r_pp)];
