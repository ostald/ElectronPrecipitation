...
#    rrate_smooth = np.array([PchipInterpolator(t, np.array([stepped_prod_t(r.T, i, ts, te) for i in t])) for r in rrate.swapaxes(0, 1)])
    ...
    def solve_ic_ss():
        res = np.empty(model.n_heights, dtype='object')
        for h in range(model.n_heights):
            temp = PchipInterpolator([0, 1], np.array([[Tn[h, ii], Tn[h, ii]],
                                                   [Ti[h, ii], Ti[h, ii]],
                                                   [Te[h, ii], Te[h, ii]]]).T)
            res[h] = solve_ivp(fun, (0, t_end), n0[h, :], method='BDF', vectorized=False, args=[h, temp],
                              t_eval=tt, atol = 1e-3, rtol = 1e-7, dense_output=True)
        return res
    ...
    n_ic_ss = np.array([r.y for r in res_ss])


    [e, O, O2, O2p, Np, N2, N2p, NO, NOp, H, Hp, O_1D, O_1S, N_2D, N_4S, O2p_a4P, Op_2D, Op_4S, Op_2P] = n_ic_ss.swapaxes(
        1, 0)

    ih = 50
    plt.figure()
    plt.plot(tt, NOp[ih, :], label = 'NO+')
    plt.plot(tt, O2p[ih, :], label = 'O2+')
    plt.xlabel("Time [s]")
    plt.ylabel("Density [m-3]")
    plt.yscale("log")
    plt.legend()
    plt.twinx()
    plt.plot(tt, NOp[ih, :] / O2p[ih, :], label = 'Ratio NO+/O2+', linestyle = '--')
    plt.legend()
...
def plot_nop_o2p_prod(N2p_prod, O2p_prod, Op_prod, model, n_ic, z_model):
    ii = 283
    plt.figure()
    for r in model.all_reactions:
        if "O2+" in r.products:
